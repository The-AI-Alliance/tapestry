...
- A governed contribution policy applies a quality floor and anti-capture cap
  before integrating accepted deltas into the shared base.
...
| `node.py` | `SovereignTrainingNode`, which runs local continued pretraining and keeps a sovereign model artifact. |
...
3. The coordinator integrates only governed weight deltas.
...
production governance, or a complete post-training alignment pipeline.
