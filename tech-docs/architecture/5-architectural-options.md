...
**Evaluation:** The [Inglehart-Welzel Cultural Map](https://www.worldvaluessurvey.org/WVSContents.jsp?CMSID=Findings) — which maps cultures onto two dimensions (traditional vs. secular-rational, survival vs. self-expression) based on the World Values Survey — provides a measurable framework for cultural alignment evaluation. A model's outputs can be assessed against its target culture's position on the map. If the model clusters with the base model's culture of origin rather than the target culture, the alignment did not work. Research using Hofstede's cultural dimensions and the WVS is already being applied to LLM evaluation ([Sukiennik 2025](https://arxiv.org/pdf/2504.08863), [Cultural Alignment in LLMs, COLING 2025](https://aclanthology.org/2025.coling-main.567.pdf)).
