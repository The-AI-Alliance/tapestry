...
| Durable `LinkState` persists infrastructure state (runs, messages, node registrations, object metadata), not training-round semantics | Flower SuperLink state-persistence and high-availability docs enumerate what is stored; nothing in it describes phase-level round checkpointing or mid-merge resume [8][9] |
| SQLite state on a network-mounted filesystem produces `database is locked` failures | Flower FAQ, on SQLite over NFS-class and cluster storage [10] |
| A round can wait indefinitely and without a logged reason when the participating node count is unmet | `FedAvg` defaults `min_train_nodes` / `min_available_nodes` to 2, and the Flower WAN spike observed `strategy.start` blocking with no message below that threshold [6][11] |
| Flower supplies server-side TLS plus optional SuperNode key authentication, which is a different mechanism from the AWS reference's client-certificate mTLS | Flower TLS setup uses `--ssl-ca-certfile` server side and `--root-certificates` in place of `--insecure` node side; SuperNode authentication is documented as a separate optional layer keyed on node keys [12][13] |
| A TLS `handshake failed` makes a SuperNode give up rather than retry | Flower's gRPC retry give-up predicate treats `UNAVAILABLE` as retryable except where the status details contain `handshake failed` [6] |
| The mutually-authenticated ingress is unexercised in Tapestry to date | The Flower WAN spike, the only Tapestry deployment of the transport so far, ran `--insecure` [11] |
...
6. Verify the durable-state and telemetry-disable requirements against the substrate actually chosen: if Flower is used, that `LinkState` is backed by a durable store rather than in-memory SQLite, and that `FLWR_TELEMETRY_ENABLED=0` is set on the central node. Where that store is SQLite on a network-mounted filesystem, confirm it is not, since SQLite on NFS-class storage produces `database is locked` failures.
7. Stand up the mutually-authenticated ingress of item 5 once, end to end, against the substrate actually chosen, and record which shape it took (client-certificate mTLS or Flower SuperNode key auth) and what the member allow-list is keyed on. The existing WAN spike ran `--insecure`, so this requirement is currently asserted rather than exercised.
8. State the M0 straggler rule for the sync barrier, even if the rule is "wait for both members, then abandon and rerun the round by hand". Give it a bounded wait and a logged cause so a missing member surfaces as an error rather than a hang. Where that rule could ever drop a contribution and still promote a base, route it through governance rather than setting it as a coordinator-side default.
9. Record which failure points in a round are restartable at M0 and which are restart-from-zero, and confirm that abandoning and rerunning a round is acceptable at M0 cost. This is the minimum needed to hold the position that round checkpoint semantics can be deferred; the design itself is tracked on [#183](https://github.com/The-AI-Alliance/tapestry/issues/183).
...
- **A durable state backend is necessary but not sufficient for multi-day rounds.** Item 2 of the M0 build requires durable coordinator state, and Flower's `LinkState` supplies it, but what it persists is *infrastructure* state: runs, messages, node registrations, object metadata. It does not define what a *training round* checkpoints. Three things stay undefined and are the coordinator's to answer, not the substrate's:
    - 1. What state is written at each phase of the loop (distribute, collect, merge, evaluate, promote), and at which points a round is restartable rather than restart-from-zero.
    - 2. How a round that died mid-merge resumes, and specifically how to determine which member contributions were in versus out at the moment of failure, since a partially-applied weighted average is not a valid base and is not distinguishable from a complete one by inspection.
    - 3. How a sovereign node learns which round it is in after a coordinator restart, so a node that was mid-round does not submit into the wrong one.
  For M0 with two known members a round is cheap enough to abandon and rerun, so this is not a blocker for the first bring-up. It becomes load-bearing as soon as a round costs enough that rerunning it is not the obvious answer. The design belongs in the round-semantics work tracked on [#183](https://github.com/The-AI-Alliance/tapestry/issues/183), not here; this ADR only records that durable `LinkState` does not discharge it.
- **The sync barrier has no straggler policy.** The Shared-Base Loop as sized here assumes a synchronous FedAvg barrier: the round advances when contributions are in. Nothing in this ADR says what happens when one sovereign node is slow, unreachable, or drops mid-round. With two known members at M0 the answer can be "wait, then rerun by hand", and that is defensible. Two things make it worth naming now rather than at M1:
    - The default is a silent one. Flower's `FedAvg` defaults to `min_train_nodes=2` / `min_available_nodes=2`, and where the participating count is not met the round waits indefinitely without emitting a reason, so a lost node presents as a hang rather than an error. Whatever substrate is used, the operator needs a bounded wait and a logged cause.
    - The choice is governance-adjacent, not purely operational. Proceeding without a member's contribution changes who is represented in that round's merged base, so a straggler policy that drops contributions silently would weight participation by network reliability. Under the DG3 and DG5 arguments above, any such policy is a governance-configurable and audited input like admission and weighting, not a coordinator-side timeout constant.
- **"Mutually-authenticated ingress" is unvalidated in Tapestry, and Flower's version of it is not the AWS reference's version.** Item 5 requires mutual auth per member, and the AWS reference reads that as X.509 mTLS. The Flower WAN spike, the only place the transport has actually been exercised, ran with `--insecure`, so no mutually-authenticated path has been stood up end to end here yet. On the substrate side the shapes differ: Flower ships server-side TLS (`--ssl-ca-certfile`, with `--root-certificates` replacing `--insecure` on the node) plus SuperNode key-based authentication as a *separate* optional layer, which satisfies "mutual auth" through node keys rather than client certificates. Either shape can meet the requirement; they are not drop-in substitutes for each other, and the choice affects what the allow-list is keyed on. One sharp edge for whoever turns it on: a TLS `handshake failed` is one of the few conditions a SuperNode gives up on immediately instead of retrying, so a certificate misconfiguration presents as fast node exits, not as reconnect churn.
...
8. Persist the State of the SuperLink: enumerates the state Flower persists (runs, messages, node registrations, object metadata) and the `--database` flag. <https://flower.ai/docs/framework/docker/persist-superlink-state.html>
9. SuperLink High Availability Mode: describes the load-balanced multi-replica architecture with a shared PostgreSQL backend. Available in preview and access-gated; described as an architecture overview rather than a deployment guide. <https://flower.ai/docs/framework/explanation-superlink-high-availability.html>
10. Flower FAQ, on SQLite state over networked filesystems producing `database is locked` errors. <https://flower.ai/docs/framework/ref-faq.html>
11. Flower WAN spike (this project): `contrib/jneums-flower-wan-spike/README.md`, which records both the `--insecure` transport and the `min_train_nodes` / `min_available_nodes` blocking behavior.
12. Enable TLS Connections: `--ssl-ca-certfile` server side, `--root-certificates` in place of `--insecure` node side. <https://flower.ai/docs/framework/how-to-enable-tls-connections.html>
13. Authenticate SuperNodes: node key-based authentication as a layer separate from transport TLS. <https://flower.ai/docs/framework/how-to-authenticate-supernodes.html>